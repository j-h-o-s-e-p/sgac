...
class StudentService:
    ...
    def get_dashboard_stats(student):
        ...
        if total_courses > 0:
            total_attendance = sum(
                [e.current_attendance_percentage for e in enrollments]
            )
    ...
    def get_grades_summary(student):
        ...
        enrollments = (
            StudentEnrollment.objects.filter(student=student, status="ACTIVO")
            .select_related("course", "group")
            .prefetch_related("grade_records__evaluation")
        )

        courses_data = []
        for enrollment in enrollments:
            evaluations = Evaluation.objects.filter(course=enrollment.course).order_by(
                "unit", "name"
            )
            grades = GradeRecord.objects.filter(enrollment=enrollment).select_related(
                "evaluation"
            )
            ...
            for unit, evals in units_data.items():
                ...
                for item in evals:
                    if item["grade"]:
                        ...
                        weighted_sum += (
                            float(item["grade"].rounded_score)
                            * float(item["evaluation"].percentage)
                            / 100
                        )
    ...
    def get_syllabus_list(student):
        ...
        enrollments = StudentEnrollment.objects.filter(
            student=student, status="ACTIVO", course__syllabus__isnull=False
    ...
    def get_syllabus_detail(student, course_id):
        ...
        completed_sessions = SessionProgress.objects.filter(
            session__syllabus=syllabus, course_group=enrollment.group
        ).count()
    ...
    def get_attendance_detail(student, course_id):
        ...
        return StudentService._calculate_attendance_metrics(
            enrollment, include_records=True
        )
    ...
    def _calculate_attendance_metrics(enrollment, include_records=False):
        ...
        justified = records_qs.filter(
            status="J"
        ).count()  # La 'J' cuenta como asistencia para el %
    ...
    def get_available_lab_campaigns(student):
        ...
        for enrollment in enrollments_no_lab:
            ...
            already_postulated = StudentPostulation.objects.filter(
                campaign=campaign, student=student
            ).first()
    ...
    def postulate_to_lab(student, campaign_id, lab_id):
        ...
        try:
            ...
            campaign = LabEnrollmentCampaign.objects.filter(
                campaign_id=campaign_id, is_closed=False
            ).first()
            ...
            enrollment = StudentEnrollment.objects.filter(
                student=student, course=campaign.course, status="ACTIVO"
            ).first()
            ...
            lab = LaboratoryGroup.objects.filter(
                lab_id=lab_id, course=campaign.course
            ).first()
            ...
            if existing:
                result["errors"].append(
                    "Ya postulaste a un laboratorio en esta campaña."
                )
                return result
            ...
            if StudentService._check_student_lab_conflict(student, lab):
                result["errors"].append(
                    "Tienes un cruce de horario con este laboratorio."
                )
                return result
    ...
    def get_student_postulations(student):
        ...
        """
        postulations = (
            StudentPostulation.objects.filter(student=student)
            .select_related(
                "campaign__course",
                "lab_group__room",
        ...
        for post in postulations:
            status_info = {
                ...
                post.status,
                {"class": "secondary", "text": post.status, "icon": "question-circle"},
            )

            data.append({"postulation": post, "status_info": status_info})
    ...
    def get_lab_details_dto(lab_id, student):
        ...
        try:
            ...
            return {"success": False, "error": f"Error al obtener detalles: {str(e)}"}
