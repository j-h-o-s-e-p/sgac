...
from infrastructure.persistence.models import (
    ...
    Course, CustomUser, Syllabus, SessionProgress, SyllabusSession
)
...
class ProfessorService:
    ...
    def get_dashboard_stats(self, user):
        ...
        """
        course_groups = CourseGroup.objects.filter(professor=user).select_related('course').distinct()
        lab_groups = LaboratoryGroup.objects.filter(professor=user).select_related('course').distinct()
        
        # Calcular estudiantes totales (evitando duplicados)
        total_students = StudentEnrollment.objects.filter(
            group__in=course_groups, status='ACTIVO'
        ...
        for group in course_groups:
            ...
            course_groups_with_progress.append({
                'group': group,
                'progress': progress
            })
    ...
    def upload_syllabus(self, course_id, professor, pdf_file):
        ...
        is_teoria = CourseGroup.objects.filter(course=course, professor=professor).exists()
        is_lab = LaboratoryGroup.objects.filter(course=course, professor=professor).exists()
    ...
    def get_attendance_session_data(self, user, group_id, date_str=None):
        ...
        if current_session:
            ...
            attendance_map = self._get_or_create_attendance_map(group, enrollments, current_session, user)
            ...
            if is_editable and group_type == 'course':
                ...
                covered = self.get_topics_covered_today_count(group, current_session['date'])
    ...
    def save_attendance_process(self, user, group_id, date_obj, session_num, post_data, ip):
        ...
        self.save_attendance_and_topics(
            enrollments, session_num, date_obj, post_data, user, ip, 
    ...
    def get_attendance_report_matrix(self, user, group_id):
        ...
        enrollments = self._get_group_enrollments(group, group_type)
        sessions = get_group_sessions(group) if group_type == 'course' else get_lab_sessions(group)
        ...
        for env in enrollments:
            ...
            for s in sessions:
                row['attendance_data'].append({
                    ...
                    'status': records_map.get(env.enrollment_id, {}).get(s['number'], '-')
                })
            matrix.append(row)
    ...
    def generate_attendance_excel(self, group, sessions, matrix):
        ...
        identifier = getattr(group, 'group_code', getattr(group, 'lab_nomenclature', ''))
    ...
    def get_grades_consolidation(self, user, group_id):
        ...
        try:
            group = CourseGroup.objects.select_related('course').get(group_id=group_id, professor=user)
                    ...
                    if raw and raw.strip():
                        try:
                            ...
                            if 0 <= val <= 20:
                                ...
                                errors.append(f"Nota fuera de rango: {enrollment.student.get_full_name()}")
                        except:
                            errors.append(f"Error formato: {enrollment.student.get_full_name()}")
                ...
                try:
                    ...
                    for col, obj in [(col_c, eval_cont), (col_e, eval_exam)]:
                        if row.get(col):
                            ...
                            if 0 <= val <= 20:
                                GradeRecord.objects.update_or_create(
                                    enrollment=enrollment, evaluation=obj,
                                    defaults={'raw_score': val, 'recorded_by': user, 'is_locked': False}
                                )
                                success += 1
    ...
    def get_professor_schedule(self, professor):
        ...
        schedule_by_day = {'LUNES': [], 'MARTES': [], 'MIERCOLES': [], 'JUEVES': [], 'VIERNES': []}
        ...
        for entry in course_schedules:
            duration = datetime.combine(date.min, entry.end_time) - datetime.combine(date.min, entry.start_time)
        ...
        for lab in lab_groups:
            duration = datetime.combine(date.min, lab.end_time) - datetime.combine(date.min, lab.start_time)
            total_duration += duration
            total_classes += 1
    ...
    def get_group_syllabus_details(self, group_id):
        ...
        all_sessions = SyllabusSession.objects.filter(syllabus=syllabus).order_by('session_number')
        completed_ids = set(SessionProgress.objects.filter(course_group=group).values_list('session_id', flat=True))
        ...
        return {
            ...
            'syllabus_data': [{'unit_name': "Desarrollo del Curso", 'sessions': session_list}]
        }
    ...
    def _find_group_by_id(self, user, group_id):
        ...
        try:
            ...
            try:
                return LaboratoryGroup.objects.get(lab_id=group_id, professor=user), 'lab'
    ...
    def _get_or_create_attendance_map(self, group, enrollments, session, user):
        ...
        if not records.exists() and session['is_today']:
            new_recs = [
                AttendanceRecord(
                    enrollment=e, session_number=session['number'],
                    session_date=session['date'], status='F',
                    professor_ip='0.0.0.0', recorded_by=user
                ) for e in enrollments
            ]
    ...
    def _calculate_group_progress(self, group):
        ...
        if total == 0: return {'percentage': 0, 'color': 'secondary'}
    ...
    def _is_within_lab_schedule(self, lab):
        ...
        start_m = (datetime.combine(date.today(), lab.start_time) - timedelta(minutes=15)).time()
        end_m = (datetime.combine(date.today(), lab.end_time) + timedelta(minutes=15)).time()
        
        if start_m <= current_time <= end_m:
            return True, "Habilitado"
        return False, f"Horario: {lab.start_time.strftime('%H:%M')} - {lab.end_time.strftime('%H:%M')}"

    def save_attendance_and_topics(self, enrollments, session_num, session_date, post_data, user, ip, group):
        ...
        with transaction.atomic():
            ...
            for enrollment in enrollments:
                ...
                if status:
                    AttendanceRecord.objects.update_or_create(
                        enrollment=enrollment, session_number=session_num,
                        defaults={'session_date': session_date, 'status': status, 'professor_ip': ip, 'recorded_by': user}
                    )
            ...
            if group:
                ...
                if ids:
                    today_count = self.get_topics_covered_today_count(group, session_date)
                    limit = 2 - today_count
                    if limit > 0:
    ...
    def _process_topics(self, topic_ids, group, date_obj, user):
        ...
        for sid in topic_ids:
            if not SessionProgress.objects.filter(course_group=group, session_id=sid).exists():
                SessionProgress.objects.create(
                    session_id=sid, course_group=group, completed_date=date_obj, marked_by=user
                )

    def get_available_topics(self, group):
        """Retorna temas no completados"""
        try: syllabus = group.course.syllabus
        except: return []
        done = SessionProgress.objects.filter(course_group=group).values_list('session_id', flat=True)
        return SyllabusSession.objects.filter(syllabus=syllabus).exclude(session_id__in=done).order_by('session_number')

    def get_topics_covered_today_count(self, group, date_obj):
        return SessionProgress.objects.filter(course_group=group, completed_date=date_obj).count()