...
class SyllabusExtractor:
    ...
    def extract_hours(self) -> Dict[str, Decimal]:
        ...
        if theory_match: hours['theory'] = Decimal(theory_match.group(1))
        if practice_match: hours['practice'] = Decimal(practice_match.group(1))
        if lab_match: hours['lab'] = Decimal(lab_match.group(1))
    ...
    def extract_thematic_content(self) -> List[Dict]:
        ...
        unit_indices = []
        for match in re.finditer(r'(PRIMERA|SEGUNDA|TERCERA)\s+UNIDAD', self.text, re.IGNORECASE):
            unit_indices.append({
                'name': match.group(0),
                'start': match.end()
            })
        ...
        for i, u_data in enumerate(unit_indices):
            start = u_data['start']
            end = unit_indices[i+1]['start'] if i+1 < len(unit_indices) else len(self.text)
            ...
            chapter_name = self.clean_text(chapter_match.group(1)) if chapter_match else u_data['name']

            # Extraer Temas 
            topics = []
            topic_matches = re.finditer(r'Tema\s+\d+:\s*(.+?)(?=(?:Tema\s+\d+:)|$|SEGUNDA|TERCERA)', unit_text, re.DOTALL)
    ...
    def process_syllabus(self, syllabus_obj) -> Dict:
        ...
        from infrastructure.persistence.models import (
            Course, Syllabus, SyllabusUnit, 
            SyllabusSession, Evaluation
        )
