...
from tests.factories import (
    StudentFactory, CourseFactory, CourseGroupFactory,
    StudentEnrollmentFactory, EvaluationFactory,
    GradeRecordFactory, ProfessorFactory
)
...
class TestCompleteEnrollmentWorkflow:
    ...
    def test_complete_student_academic_cycle(self):
        ...
        group = CourseGroupFactory.create(
            course=course,
            professor=professor
        )
        ...
        assert attendance_decimal == Decimal("83.33"), f"Esperado 83.33, obtenido {attendance_decimal}"

        # Verificar nota final: (17*0.30) + (14*0.40) + (19*0.30) = 16.40
        final_grade_decimal = to_decimal(final_grade)
        assert final_grade_decimal == Decimal("16.40"), f"Esperado 16.40, obtenido {final_grade_decimal}"
        ...
        assert db_attendance == Decimal("83.33"), f"En BD: Esperado 83.33, obtenido {db_attendance}"

        # Verificar nota final en BD
        db_final_grade = to_decimal(enrollment.final_grade)
        assert db_final_grade == Decimal("16.40"), f"En BD: Esperado 16.40, obtenido {db_final_grade}"
        ...
        def assert_decimal_equal(actual, expected, tolerance=Decimal('0.01')):
            ...
            assert difference <= tolerance, f"Esperado {expected_dec}, obtenido {actual_dec} (diferencia: {difference})"
