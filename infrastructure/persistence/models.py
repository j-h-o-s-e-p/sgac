...
class CustomUser(AbstractUser):
    ...
    account_status = models.CharField(
        max_length=20, choices=USER_STATUS_CHOICES, default="INACTIVO"
    )
...
class ExternalProfessor(models.Model):
    ...
    external_prof_id = models.UUIDField(
...
class Classroom(models.Model):
    ...
    code = models.CharField(
        max_length=10, unique=True, editable=False, blank=True, verbose_name="Código"
    )
...
class CourseGroup(models.Model):
    ...
    class Meta:
        ...
        unique_together = [
            ["course", "group_code"]
        ]  # No puede haber dos grupos 'A' en el mismo curso
...
class Syllabus(models.Model):
    ...
    syllabus_file = models.FileField(
        upload_to="syllabi/%Y/%m/", blank=True, null=True, verbose_name="PDF del Sílabo"
    )

    # Metadata sacada automáticamente al leer el PDF
    credits_extracted = models.IntegerField(
        null=True, blank=True, verbose_name="Créditos extraídos"
    )
    ...
    evaluations_configured = models.BooleanField(
    ...
    def get_progress_percentage(self):
        ...
        completed_sessions = SessionProgress.objects.filter(
            session__syllabus=self, session__isnull=False
        ).count()
...
class SyllabusUnit(models.Model):
    ...
    syllabus = models.ForeignKey(
        Syllabus, on_delete=models.CASCADE, related_name="units"
    )
...
class SyllabusSession(models.Model):
    ...
    syllabus = models.ForeignKey(
        Syllabus, on_delete=models.CASCADE, related_name="sessions"
    )
    ...
    real_date = models.DateField(
        null=True, blank=True, verbose_name="Fecha Real de Ejecución"
    )

    accumulated_percentage = models.DecimalField(
...
class SessionProgress(models.Model):
    ...
    session = models.OneToOneField(
        SyllabusSession, on_delete=models.CASCADE, related_name="progress"
    )
    course_group = models.ForeignKey(
        CourseGroup, on_delete=models.CASCADE, related_name="session_progress"
    )

    completed_date = models.DateField()
    marked_by = models.ForeignKey(
        CustomUser, on_delete=models.SET_NULL, null=True, related_name="sessions_marked"
    )
...
class LabEnrollmentCampaign(models.Model):
    ...
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, related_name="lab_campaigns"
    )
...
class StudentPostulation(models.Model):
    ...
    postulation_id = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False
    )
    campaign = models.ForeignKey(
        LabEnrollmentCampaign, on_delete=models.CASCADE, related_name="postulations"
    )
    ...
    lab_group = models.ForeignKey(
        LaboratoryGroup, on_delete=models.CASCADE, related_name="postulations"
    )
    timestamp = models.DateTimeField(auto_now_add=True)

    status = models.CharField(
        max_length=20, choices=POSTULATION_STATUS_CHOICES, default="PENDIENTE"
    )
...
class LabAssignment(models.Model):
    ...
    postulation = models.OneToOneField(
        StudentPostulation, on_delete=models.CASCADE, related_name="assignment"
    )
    student = models.ForeignKey(
        CustomUser,
        on_delete=models.CASCADE,
    ...
    lab_group = models.ForeignKey(
        LaboratoryGroup, on_delete=models.CASCADE, related_name="assignments"
    )
...
class StudentEnrollment(models.Model):
    ...
    lab_assignment = models.ForeignKey(
        LabAssignment,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    ...
    status = models.CharField(
        max_length=20, choices=ENROLLMENT_STATUS_CHOICES, default="ACTIVO"
    )

    current_attendance_percentage = models.DecimalField(
    ...
    def calculate_final_grade(self):
        ...
        grade_records = self.grade_records.filter(
        ...
        for evaluation in evaluations:
            ...
            grade_record = next(
                (
                    ...
                    if gr.evaluation_id == evaluation.evaluation_id
                ),
                None,
            )
...
class GradeRecord(models.Model):
    record_id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    enrollment = models.ForeignKey(
        StudentEnrollment, on_delete=models.CASCADE, related_name="grade_records"
    )
    evaluation = models.ForeignKey(
        Evaluation, on_delete=models.CASCADE, related_name="grade_records"
    )
