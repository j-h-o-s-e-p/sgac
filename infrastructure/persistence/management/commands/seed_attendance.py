...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=str,
            default="1703237",
            help="Código del curso (Default: IS2)",
        )
    ...
    def handle(self, *args, **options):
                    ...
                    if sesion["date"] <= hoy:
                        for env in enrollments:
                            if not AttendanceRecord.objects.filter(
                                enrollment=env, session_number=sesion["number"]
                            ).exists():
                                AttendanceRecord.objects.create(
                                    enrollment=env,
                                    session_number=sesion["number"],
                                    session_date=sesion["date"],
                                    status=random.choices(opciones, weights=pesos, k=1)[
                                        0
                                    ],
        ...
        for lab in labs:
            ...
            enrollments = StudentEnrollment.objects.filter(
                lab_assignment__lab_group=lab, status="ACTIVO"
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✅ LISTO: {total} registros creados para el curso {codigo_curso}."
            )
        )
