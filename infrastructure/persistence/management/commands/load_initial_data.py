...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(f"--- CARGANDO DATOS DESDE: {csv_path} ---")
        )

        if not os.path.exists(csv_path):
            self.stdout.write(
                self.style.ERROR(f"Error: El archivo {csv_path} no existe.")
            )
            return
                    ...
                    for row_num, row in enumerate(reader, start=2):
                        try:
                            ...
                            if docente_nombre and email:
                                professor = self.get_or_create_professor(
                                    docente_nombre, email
                                )
                                if (
                                    professor
                                    and professor.date_joined.date() == date.today()
                                ):
                                    stats["professors"] += 1
    ...
    def get_or_create_active_semester(self):
        ...
        if created:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Semestre {sem.name} creado y activado.")
            )
        return sem
