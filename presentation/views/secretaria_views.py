...
class ClassroomListView(SecretariaRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if status_filter:
            reservations = (
                ClassroomReservation.objects.filter(status=status_filter)
                .select_related("classroom", "professor", "approved_by")
                .order_by("-reservation_date", "-start_time")[:50]
            )
        else:
            # Por defecto: pendientes primero, luego las más recientes
            pending = (
                ClassroomReservation.objects.filter(status="PENDIENTE")
                .select_related("classroom", "professor")
                .order_by("reservation_date", "start_time")
            )

            others = (
                ClassroomReservation.objects.exclude(status="PENDIENTE")
                .select_related("classroom", "professor", "approved_by")
                .order_by("-reservation_date", "-start_time")[:30]
            )

            reservations = list(pending) + list(others)

        context["classroom_reservations"] = reservations
        context["pending_reservations_count"] = ClassroomReservation.objects.filter(
            status="PENDIENTE"
        ).count()
...
class CourseScheduleView(SecretariaRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["classrooms"] = Classroom.objects.filter(is_active=True).order_by(
            "name"
        )
...
def upload_students_to_group_view(request, group_id):
    ...
    if result["success"]:
        ...
        messages.success(
            request,
            f"Procesado: {stats['created']} nuevos, {stats['enrolled']} matriculados en {g.group_code}.",
        )
    else:
...
class SecretariaGradeReportView(SecretariaRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["groups"] = CourseGroup.objects.select_related(
            "course", "professor"
...
def reject_reservation_api(request):
    ...
    try:
        ...
        result = SecretariaService.reject_reservation(
            reservation_id, request.user, reason
        )
