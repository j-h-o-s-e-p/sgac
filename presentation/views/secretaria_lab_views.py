...
from infrastructure.persistence.models import (
    LaboratoryGroup, Classroom, CustomUser, 
    ExternalProfessor, CourseGroup
)
...
class LabManagementView(SecretariaRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for course in courses_with_lab:
            ...
            theory_ids = list(CourseGroup.objects.filter(
                course=course, 
...
class CheckScheduleConflictsView(SecretariaRequiredMixin, View):
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse({
                'has_conflict': True, 
                'messages': [f"Error de validación: {str(e)}"]
            }, status=400)
...
class CloseLabEnrollmentView(SecretariaRequiredMixin, View):
    ...
    def post(self, request, course_id):
        ...
        if result['success']:
            messages.success(request, "Inscripción cerrada y alumnos asignados correctamente")
