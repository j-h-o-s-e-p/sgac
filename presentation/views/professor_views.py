    ...
    if request.method == 'POST':
        ...
        if session and context['is_editable']:
            success = _service.save_attendance_process(
                request.user, group_id, session['date'], session['number'], 
                request.POST, request.META.get('REMOTE_ADDR')
            )
            if success: messages.success(request, 'Asistencia guardada correctamente.')
            else: messages.error(request, 'Error al guardar.')
...
def attendance_report(request, group_id):
    ...
    if not data: return redirect('presentation:professor_dashboard')

    # Exportación Excel
    if request.GET.get('export') == 'excel':
        wb = _service.generate_attendance_excel(data['group'], data['sessions'], data['matrix'])
...
def consolidated_grades(request, group_id):
    ...
    if not context: return redirect('presentation:professor_dashboard')

    # 2. Guardar Notas (POST)
    if request.method == 'POST':
        unit = int(request.POST.get('unit_number'))
        saved, errors = _service.save_grades_batch(context['course'], unit, request.POST, request.user)
...
def upload_grades_csv(request, course_id):
    ...
    if request.method == 'POST' and request.FILES.get('csv_file'):
        ...
        try:
            success, errors = _service.process_csv_grades(request.FILES['csv_file'], course, unit, request.user)
            if errors: messages.warning(request, f"Cargados: {success}. Errores: {len(errors)}")
            else: messages.success(request, f"Éxito: {success} notas cargadas.")
        except Exception as e:
            messages.error(request, f"Error: {str(e)}")
            
    # Redirige atrás
    return redirect(request.META.get('HTTP_REFERER', 'presentation:professor_dashboard'))
...
def upload_syllabus(request, course_id):
    ...
    if request.method == 'POST' and request.FILES.get('syllabus_pdf'):
        try:
            _service.upload_syllabus(course_id, request.user, request.FILES['syllabus_pdf'])
...
def get_course_progress_api(request, group_id):
    ...
    return JsonResponse(data if data else {'error': 'No encontrado'}, status=200 if data else 404)