...
def classroom_reservation(request):
    ...
    active_reservations = _service.get_professor_reservations(
        request.user, include_history=False
    )
    history = _service.get_professor_reservations(request.user, include_history=True)[
        :20
    ]
    ...
    stats = {
        ...
        "pending": ClassroomReservation.objects.filter(
            professor=request.user, status="PENDIENTE"
        ).count(),
        "approved": ClassroomReservation.objects.filter(
            professor=request.user, status="APROBADA"
        ).count(),
    }
...
def create_reservation_api(request):
    ...
    try:
        ...
        result = _service.create_classroom_reservation(
            request.user, classroom_id, date_obj, start_time, end_time, purpose
        )
